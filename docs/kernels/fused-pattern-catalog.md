...
| `fused_radam_step` | 5+host | RAdam (AdamW + bias correction + denom switch) | `crates/trainer/src/optimiser/radam.rs::RAdamParams` | `pointwise/radam_step.rs` | `experiments/002-fused-kernels/src/main.rs::radam_step` | #40 | 実装済み (PR pending) |
| `fused_ranger_step` | RAdam + lookahead | Ranger (RAdam + slow params lerp、k-step periodic) | `crates/trainer/src/optimiser/ranger.rs` | `pointwise/ranger_step.rs` | `experiments/002-fused-kernels/src/main.rs::ranger_lookahead_lerp` (+ Stage 2-4 `radam_step` 再利用) | #41 | 実装済み (PR pending) |
