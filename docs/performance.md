...
| `--ft-fp16 --ft-fp16-out --fp16-opt-state` | ~1.44M | +108% | + optimizer の m/v を FP16 保持 |

`--fp16-opt-state` は他 2 つと独立した軸で、どの構成にも積める。単独 (FP32 +
`--fp16-opt-state`) では ~962K (+1.3%)、`--ft-fp16 --ft-fp16-out` の上では ~1.44M
(+1.9%)。各モードの精度設計と phase 別内訳は下記「FP16 FT weight モード」「FP16 FT
activation モード」「FP16 optimizer state モード」節を参照。
...
| `optimizer` | ~4.5 | `radam_step` × 10 + `ranger_lookahead_lerp` × 10。`--fp16-opt-state` で ~3.4 (`ft_w` の m/v を FP16 化し DRAM traffic 削減) |
...
## FP16 optimizer state モード (`--fp16-opt-state`)

`--fp16-opt-state` を付けると、FT weight (`ft_w`) の Ranger optimizer moment
(1st moment `m` / 2nd moment `v`) を FP16 で保持する。既定 OFF では FP32 path と
bit-identical。`--ft-fp16` / `--ft-fp16-out` とは独立した flag で、どの構成にも積める。

`ft_w` は FT_IN×FT_OUT = 73,305×1536 ≈ 112.6M 要素あり、その optimizer step
(`radam_step`) は `m` / `v` の read+write が DRAM 帯域律速。`m` / `v` を半精度化すると
`optimizer` phase の DRAM traffic が減る。RTX 3080 Ti での profile-on 計測:

| 指標 (RTX 3080 Ti) | `--fp16-opt-state` OFF | ON |
|---|---:|---:|
| `optimizer` (profile-on) | ~4.5 ms | ~3.4 ms |
| pos/s (FP32 構成) | ~949K | ~962K |
| pos/s (`--ft-fp16 --ft-fp16-out` 構成) | ~1.41M | ~1.44M |

精度設計: `m` / `v` は batch 正規化された勾配由来で値が極めて小さく (実測で `|m|`
中央値 ~1e-9、`v` 中央値 ~1e-15)、無 scale で FP16 化すると大半が subnormal 下限
(2^-24 ≈ 6e-8) を下回り潰れる。FP16 へ書く前に固定の power-of-2 scale (`m`: 2^28、
`v`: 2^40) を掛けて normal range に持ち上げ、読み出し時に割り戻す。`m` / `v` は値域の
オーダーが約 `m`/`m^2` の関係で異なるため別 scale。scale 後も f16 有限域を超えうる
外れ値は格納前に clamp する。raw checkpoint (`.ckpt`) は mode に依らず常に真値 f32 で
保存するため、`--fp16-opt-state` あり/なしを跨いで resume できる。

loss 軌跡は FP32 と差 ~1e-5 (sb5、FP32 の run 間ばらつきと同程度) だが、量子化誤差で
棋力が変動しうるため既定 OFF、本番品質は SPRT 確認まで保証しない。
