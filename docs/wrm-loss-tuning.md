...
`--scale = --wrm-nnue2score` whether or not weights are loaded. To tune
`--wrm-nnue2score` independently of `--scale`, set the measured export scale
with `simple --fv-scale <N>`. A v9+ raw checkpoint saves that value, so it needs
no respecification; when resuming v8 or earlier, repeat the option or the value
falls back to the `--scale`-derived value.
