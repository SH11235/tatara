...
`--scale`, set the measured export scale with `simple --fv-scale <N>`. Raw
checkpoints do not store `fv_scale`, so repeat `simple --fv-scale <N>` on every
`--resume` invocation that must preserve the chosen value.
