...
`docs/experiments/` 配下は計測ログの history doc 専用で本 checklist の対象外。
...
rg -n --type rust --type-add 'cfg:*.{toml,yaml,yml,sh,md}' --type cfg \
   ...
   -g '!docs/experiments/**' -g '!target/**' -g '!.git/**'
```
...
削除済 directory (`experiments/`) や架空の予定 directory が残っていないか。
...
```bash
rg -n --type rust 'bullet 上流' -B1 -A2 -g '!docs/experiments/**'
...
rg -oE '\`?(crates|bins|docs|experiments|scripts)/[a-zA-Z0-9_/-]+\.(rs|md|toml|sh|yaml)\`?' \
