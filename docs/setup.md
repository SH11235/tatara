...
| Windows | WSL2 (Ubuntu) 経由でサポート。native Windows ビルドは cuda-oxide が未検証 | 先に「Windows (WSL2) の準備」、以降は Linux と共通 |
...
native Windows 上の cuda-oxide ビルドは upstream が検証していない (cuda-oxide
は Linux でテストされている)。Windows では **WSL2 + Ubuntu** を使う。WSL2 から
は NVIDIA GPU が CUDA 経由で見えるため、WSL2 内では本ファイルの Linux 手順が
そのまま通る。
...
する。エラーなく学習が始まれば pipeline は通っている。
