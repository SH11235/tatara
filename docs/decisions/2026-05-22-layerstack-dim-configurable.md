...
### 2. custom kernel は `cublasSgemm` 置換でなく tile-loop + dynamic shared memory で一般化する
...
方式: tile 定数 (TILE_OUT=16 等) は compile-time const のまま保ち、`L1_OUT` /
`L2_OUT` は runtime の「tile 数」loop bound にする。tile の shared memory は
`DynamicSharedArray` で launch 時に実サイズを確保する。`SharedArray` を対応上限
まで over-allocate する案は、既定構成 (本番) でも上限サイズの shared memory を
取って occupancy を落とすため採らない。既定値では tile 1 個分のループ = 従来と
同一の計算順序になり bit-identical。const generic で値ごとの kernel を自動生成
する案は cuda-oxide 非対応 (上記 Context) のため不可。restructure が困難な kernel
が出た場合の fallback は、`macro_rules!` で許容値ごとの named kernel を生成する
enum 特殊化とする。

各 kernel の具体的な restructure 可否は実装時の kernel 調査で確定する。本項は
方向と cuda-oxide 制約を定めるもので、per-kernel の戦術は含まない。
