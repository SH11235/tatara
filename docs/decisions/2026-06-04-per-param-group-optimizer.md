...
直近の optimizer launch の table-drive 化で、`radam_step` / `ranger_lookahead_lerp` の
launch は一様 weight group を `UniformOptimGroup` 配列で回す形に集約された。`radam_step`
...
per-group の `(weight_decay, lr)` を resolve し、`UniformOptimGroup` に per-group の
weight_decay / lr を持たせて radam loop で `g.weight_decay` / `g.lr` を渡す。FT
...
- **bias wd=0 を既定にする**: nnue-pytorch の定石に寄せられるが、既定経路の
  bit-identical (§4 の既定方針) を破る。opt-in に留める。却下。
