...
```jsonc
{
  ...
  "id": "v102-20260517t041530z",     // net_id + UTC 開始時刻、run 一意 (※2)
  "name": "v102",                     // 既定 = net_id (resume run は別既定、※3)
  ...
  "lineage": {                        // ※5  --resume した run のみ
    "parent_id": "v102-20260516t221000z",
    "resumed_from_checkpoint": "v102-200.ckpt",
    "resumed_from_superbatch": 200
  },

  "params": {                         // passthrough — 未知キーも保持される
    "architecture": "v102-LayerStack-1536-16-32-9bucket",
  ...
  "checkpoints": ["v102-20.bin", "v102-20.ckpt", "v102-40.bin"]  // ※12
}
```

- **※1 `generator`**: producer (rshogi-nnue / bullet-shogi) の区別用。top-level
  なので未改修の `nnue-lab` では正規化時に削除される (reject はされない)。
- **※2 `id`**: `{net_id}-{UTC開始時刻}` で run を一意化する。同一ホスト上で
  同じ net を同一秒に 2 回開始することは実運用で起きないため衝突しない。
  `nnue-lab` は取り込み時に独自 DB id (ulid) を採番するため、本 `id` は人間
  可読の相関キー兼 lineage の親参照キーとして機能する (連携契約を参照)。
