...
`schema_version` は常に明示出力する。値は `nnue-lab` schema 契約の version で、
held-out validation メトリクスの追加に伴い現在 `2` (下記「schema_version 2」節)。
producer 自身の version は `generator.version` が別に持つ。

注釈付きの骨子 (値は例。フィールドは ADR 決定時点の抜粋で、その後の機能追加で
増えたフィールドは載せていない — 全フィールドの真実源は
`crates/nnue-train/src/experiment.rs` の serialize 実装):
...
- **※11 `history` 要素**: `nnue-lab` の history 要素 schema は passthrough で
  ないため、要素に足したキーは正規化時に削除される。schema v1 は `history` を
  `{superbatch, loss}` に限定し、集約値 (`mean_pos_per_sec` 等) を `results` 側に
  置いた。schema v2 は held-out validation の per-superbatch メトリクス
  (`test_loss` / `test_accuracy`) を history 要素に足す — history 要素を例外的に
  拡張するため `nnue-lab` 側の coupled な schema 変更を要する (下記
  「schema_version 2」節)。
...
## schema_version 2 — held-out validation メトリクス

held-out validation (学習に使わない局面での per-superbatch 検証) の出力に伴い
`schema_version` を 2 に上げ、次の optional フィールドを追加する。いずれも
held-out source (`--test-data` / `--test-tail-positions`) 未指定の run では
出力されず、held-out validation 無しの v2 experiment.json は v1 と同一の内容に
なる。

- `params.test_data` / `params.test_positions` — 検証 PSV の basename と検証
  局面数。`params` は passthrough なので未改修 `nnue-lab` でも保持される。
- `results.best_test_loss` / `results.best_test_loss_superbatch` — run 中で最良
  (最小) の held-out loss と、その superbatch。集約値なので `results` に置く
  (※10 / ※11 の方針どおり)。`results` は passthrough、保持される。
- `history[]` 要素の `test_loss` / `test_accuracy` — per-superbatch の検証 loss
  と sign-agreement accuracy (出力符号と対局結果の一致率、引き分け除外)。

`history[]` 要素は passthrough でない (※11) ため、`test_loss` / `test_accuracy`
は未改修の `nnue-lab` では取り込み時に削除される。これは `generator` / `lineage`
の degrade (連携契約を参照) と同じ扱いで、ファイル自体は取り込め、per-superbatch
の検証系列だけが失われる。系列を `nnue-lab` まで届かせるには history 要素 schema
の拡張 (passthrough 化、または `test_loss` / `test_accuracy` の明示追加) が要る。
これは producer (本リポ) の v2 出力と coupled な `nnue-lab` 側変更で、別途
`nnue-lab` 側 PR で行う。

検証メトリクスを `results` でなく `history` に置くのは、per-superbatch 値の自然な
置き場が `history` であり、`results` を per-superbatch 配列で持たせるのは
「run 全体の集約」という `results` の意味と整合しないため。集約済みの
`best_test_loss` のみ `results` に置く。
...
- per-superbatch の throughput / lr 系列を残す要求が出た場合、`nnue-lab` の
  history 要素 schema を passthrough 化する変更とセットで検討する。held-out
  validation の `test_loss` / `test_accuracy` は schema v2 でこの形 (history 要素
  拡張 + `nnue-lab` 側 coupled 変更) を採った (上記「schema_version 2」節)。
  `nnue-lab` 側の history schema 拡張 PR が残課題。
