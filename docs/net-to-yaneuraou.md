...
`fv_scale` token を省略する。省略した net を使うときは、読み込み前に YaneuraOu または
rshogi へ `setoption name FV_SCALE value N` を指定する必要がある。rshogi は token が
なくてもエラーにせず `FV_SCALE_HALFKA=16` へ fallback するため、setoption を忘れると
評価値が意図せず raw score ÷ 16 になる。`net_from_yo` は YaneuraOu SFNN から値を復元
できないため、生成する tatara net には従来値の `fv_scale=28` を書く。
