//! v102 LayerStack arch (1536-16-32 + progress8kpabs 9 buckets) で使う新規
//! kernel の reference CPU 実装。
//!
//! GPU 側 `#[kernel]` 定義は **`bins/nnue_train/src/main.rs` に inline 配置**
//! されている (cuda-oxide rustc-codegen-cuda backend の bin-entry 制約、Stage 1-5
//! で確立)。本 module 群はその `#[kernel]` のロジックを host で素直に書き写した
//! `*_cpu` 関数で、`bins/nnue_train` の `#[cfg(test)] mod gpu_cpu_equivalence_tests`
//! が GPU↔CPU 数値同等性テストの reference として使う (Issue #85)。
//!
//! bullet 上流の対応箇所は `bins/nnue_train/src/main.rs` の module doc /
//! ATTRIBUTION (`examples/shogi_layerstack.rs:2206-2289`、`crates/trainer/src/
//! model/builder.rs`) に記載。各 `*_cpu` の docstring でも触れる。
//!
//! ## kernel ↔ CPU reference 対応表 (Issue #85)
//!
...
//!
//! 既存 (Stage 1/2/#84) の reference: `loss_wdl` / `loss_wrm` / `sparse_ft_forward`
//! / `sparse_ft_backward` / `screlu_grad` / `radam_step` / `ranger_step` /
//! `adamw_step` は `pointwise/` / `sparse/` 配下にある (重複定義しない)。
//!
//! ## v102 アーキテクチャ定数 (bullet 由来、`bins/nnue_train/src/main.rs:1346-1359`)
//!
